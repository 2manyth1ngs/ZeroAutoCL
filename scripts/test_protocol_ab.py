...
forecasting:
  ...
  fixed    : crop_len=2048, batch_size=16, prefix-encoding eval (with val)

Both runs use ``pretrain_iters=600`` so the only differences are the §10
protocol changes.  We compare per-horizon MSE/MAE plus the wall-clock cost
and write everything to ``outputs/test_protocol_ab/``.
...
def write_diff(baseline: Dict, fixed: Dict, path: str) -> None:
    ...
    push(f"  baseline.mean_mse should land near 0.36–0.38 (the existing 4090 result was 0.367).")
    push(f"  observed = {baseline['test_mean_mse']:.4f}  →  "
         + ("OK" if 0.30 <= baseline['test_mean_mse'] <= 0.42 else "OUT OF RANGE — investigate"))
    push("")
    push("Expected (CLAUDE_ADV.md §10.4):")
    push("  fixed.test_mean_mse should be ≤ 0.30  (predicted gain −0.05~−0.13)")
    push("  fixed.test_per_horizon[24].mse should be ≤ 0.13  (prefix encoding helps short H most)")
...
def main() -> None:
    ...
    # Revised fixed group: align crop with TS2Vec's actual default
    # max_train_length=3000, with batch=32 (the largest batch that still
    # fits on a single 4090 24 GB card under the InfoNCE-on-temporal
    # similarity matrix regime).  See Bug #001 / §10.1 follow-up.
    fixed = run_group(
        label="fixed",
        window_len=3000,
        batch_size=32,
