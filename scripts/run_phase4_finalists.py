"""Phase 4 of Plan B: retrain Cartesian finalists on the target dataset.
...
and for each ``(encoder_config, strategy)`` pair:
  ...
  2. runs ``contrastive_pretrain`` on the target's train split using the
     budget from ``two_stage_search.finalists`` in ``configs/default.yaml``
  3. evaluates with the §10.3 prefix-encoding ``eval_forecasting`` (passing
     ``val_data`` so the train/val/test boundary is encoded jointly)
  4. records per-horizon MSE/MAE plus the mean

Writes ``phase4_results.json`` (full table) and ``phase4_best.json`` (single
best record by mean MSE) into ``--save_dir``.

Usage
-----
python scripts/run_phase4_finalists.py \\
    --finalists outputs/full_etth1_two_stage/finalists.json \\
    --target    ETTh1 \\
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Plan B Phase 4: Cartesian retrain.")
...
def _retrain_one(
    ...
    """Train one finalist and evaluate it on the target test split."""
    ...
    if not metrics:
        raise RuntimeError("eval_forecasting returned no horizons")
...
def main() -> None:
    ...
    fin_cfg = ts_cfg.get("finalists", {}) or {}

    pretrain_iters = int(fin_cfg.get("pretrain_iters", 600))
    pretrain_lr    = float(fin_cfg.get("pretrain_lr",  sg_cfg.get("pretrain_lr", 1e-3)))
    batch_size     = int(fin_cfg.get("batch_size",     sg_cfg.get("batch_size",  32)))
    ...
    logger.info(
        ...
        args.target, tuple(train_ds.data.shape),
    ...
    for i, finalist in enumerate(finalists):
        ...
        tag = (f"L{enc_cfg['n_layers']}_H{enc_cfg['hidden_dim']}"
        ...
        logger.info(
            "[phase4 %d/%d] %s mean_mse=%.4f mean_mae=%.4f train=%.0fs eval=%.0fs",
            i + 1, len(finalists), tag,
            r["mean_mse"], r["mean_mae"], r["train_secs"], r["eval_secs"],
        )

        # Free memory between finalists.
        if device.type == "cuda":
            torch.cuda.empty_cache()
    ...
    # Pick best by mean_mse (lower = better).
    ok = [r for r in results if "mean_mse" in r]
    if ok:
        ...
        logger.info(
            "[phase4] BEST: %s mean_mse=%.4f mean_mae=%.4f",
            best["tag"], best["mean_mse"], best["mean_mae"],
        )
